...
void Wind::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(Events::Event_CALCULATE_TRANSFORMATIONS, Wind::do_CALCULATE_TRANSFORMATIONS);
}

void Wind::do_CALCULATE_TRANSFORMATIONS(Events::Event *pEvt)
{
...
PrimitiveTypes::Bool Wind::castsShadow(){
	return isTheShadowCaster;
}
void Wind::setCastsShadow(PrimitiveTypes::Bool b){
	isTheShadowCaster=b;
	if(b==true){
